...
다만 `int`라서 SQL Injection엔 안전해도 **산술 오버플로**는 별개 문제다. `page=3&size=2000000000`
처럼 큰 값이 들어오면 `(page-1)*size`가 int 범위를 넘겨 음수가 되고,
`LIMIT -294967296, 2000000000` 같은 잘못된 SQL이 나가서 500이 난다. `${}`에 계산식을
넣을 땐 Service에서 `size` 상한과 `(long)(page-1)*size` 오버플로 검증을 먼저 해줘야 한다.

### `LIKE` 검색어의 와일드카드(`%`, `_`) 이스케이프

`LIKE CONCAT('%', #{keyword}, '%')`처럼 `#{}`로 바인딩해도, 검색어 안의 `%`/`_`는
SQL Injection과는 별개로 **LIKE 패턴 문자**로 해석된다(`%`는 임의 문자열, `_`는 임의
문자 1개). 검색어로 `%`를 넣으면 패턴이 `'%%%'`가 돼서 전부 매칭된다. MySQL LIKE의
기본 escape 문자가 `\`라서, 넘기기 전에 `\` -> `\\`, `%` -> `\%`, `_` -> `\_` 순서로
치환해서 리터럴로 취급되게 한다 (`\`를 가장 먼저 치환해야 방금 붙인 escape 문자가
다시 치환되지 않음).
